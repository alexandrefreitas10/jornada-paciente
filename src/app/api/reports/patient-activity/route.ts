...
export interface Activity {
  type: 'saida' | 'foto' | 'exame' | 'dieta' | 'prescricao' | 'medicao' | 'tarefa' | 'resumo'
  description: string
  detail: string | null
  created_at: string
...
export async function GET(req: NextRequest) {
  ...
  const [saidas, arquivos, medicoes, tarefas, resumos] = await Promise.all([
    ...
    `,
    // Resumos de evolução
    sql<{ patient_id: number; created_at: string; created_by: string | null }[]>`
      SELECT patient_id, created_at, created_by
  ...
  const result: PatientActivity[] = patients.map(p => {
    ...
    arquivos.filter(r => r.patient_id === p.id).forEach(r => {
      activities.push({
        type: r.file_type as Activity['type'],
