...
function SaidaForm() {
  ...
  function reset() {
    setStep('confirm'); setQuantity(1); setPatientId(''); setPatientSearch(''); setObservation(''); setError('')
  }
        ...
        <div className="px-6 py-6 text-center">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => window.close()}
              className="flex-1 py-3.5 border-2 border-gray-200 text-gray-600 font-semibold rounded-xl hover:bg-gray-50 transition-colors text-sm">
              ❌ Não
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 flex items-start justify-center p-4 pt-6">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="bg-violet-600 px-6 py-4 text-white">
          <button onClick={() => setStep('confirm')} className="text-xs opacity-75 hover:opacity-100 mb-2 flex items-center gap-1">← Voltar</button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg p-8 text-center">
        ...
        <p className="text-gray-400 text-sm mt-1">{quantity} {item.unit}{selectedPatient ? ` · ${selectedPatient.name}` : ''}</p>
